...
| `RASTA_GEOCODER_COUNTRY` | `lib/geocode.js` | `in` | Country code bias for place search (empty for worldwide) |
...
| `POST` | `/api/route` | `{from:{lat,lng}, to:{lat,lng}, persona}` → OSRM alternatives scored against walked segments, with `coverage`, `worst_hazard`, `persona_blockers`, per-persona `times`/`passable`/`blockers`, `problems` (pinned hazards), `best_for` per persona, `recommended_index`, `speeds_mps` and `candidates` (segments loaded by the bbox pre-filter). Falls back to `data/demo-route.json` if OSRM fails or exceeds 3 s |
...
| `GET` | `/api/geocode?q&lat&lng&lang` | Place search → `results[{name, display_name, lat, lng, type}]`, biased to the map centre. Rate limited like routing |
| `GET` | `/api/geocode/reverse?lat&lng&lang` | Nearest named place for a point → `place{name, display_name}` or null |
...
## Route tab: places, per-persona times, problems, walk-along

- **Type a place, not coordinates.** A and B accept place names (`/api/geocode`, Nominatim-compatible; `RASTA_GEOCODER_URL` to self-host, `RASTA_GEOCODER_COUNTRY` to bias, default India). Search runs on Enter or the Search button, never per keystroke, and results are cached for an hour with a one-request-per-second queue to respect the public Nominatim policy. Map taps still work and are labelled by reverse lookup. B can be chosen before A; Swap flips them.
- **One time per persona on every route.** `times: {walk, wheelchair, senior}` come from free speeds of 1.35 / 1.0 / 0.9 m/s plus a detour penalty per hazard whose risk for that persona is 3 or more (8 / 25 / 20 s per severity point). `passable` says whether the route has a blocking segment for that persona; blocked routes show "blocked" instead of a time. `best_for` names the best route index for each persona (highest score among ≥40%-covered passable routes, then covered, then quickest); `recommended_index` follows the selected persona and switching persona re-ranks without a new request.
- **Problems on the route.** Each route lists its recorded hazards (`problems`, worst first, with segment name and position) and pins them on the map as numbered markers; clicking a problem flies to its pin.
- **Start walking.** Zooms to A and follows the route at the chosen persona's pace with a moving marker, a progress bar, the time left and the next recorded problem ahead; a toast fires as each problem comes within 40 m. Honours `prefers-reduced-motion`.
