...
(async () => {
  ...
  try {
    ...
    page.innerHTML = `
      ...
      <table><thead><tr><th>#</th><th>Hazard</th><th>Observation</th><th>Standard</th><th>Authority</th><th>Status</th><th class="n">Severity</th><th class="n">Est. cost</th></tr></thead>
      <tbody>${seg.hazards.map((h, i) => `<tr><td>${i + 1}</td><td>${esc(types[h.type_id]?.label_en || h.type_id)}</td><td>${esc(h.note)}</td><td>${esc(types[h.type_id]?.standard_ref || '')}</td><td>${esc(h.authority || '')}</td><td>${{ observed: 'Observed', present: 'Still present', cleared: 'Reported cleared', recheck: 'Needs rechecking' }[h.lifecycle?.key] || 'Present'}${h.observed_at ? '<br><span style="color:var(--faint)">' + new Date(h.observed_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) + '</span>' : ''}</td><td class="n">${h.severity} / 5</td><td class="n">${h.cost_inr ? inr(h.cost_inr) : 'enforcement'}</td></tr>`).join('') || '<tr><td colspan="8">No hazards recorded.</td></tr>'}</tbody>
      <tfoot><tr><th colspan="7">Total indicative cost</th><th class="n">${inr(seg.total_cost_inr)}</th></tr></tfoot></table>
